...
dtms <- removeSparseTerms(dtm, 0.7) # This makes a matrix that is only 15% empty space.
library(cluster)   
d <- dist(t(dtms), method="euclidian")   # First calculate distance between words
fit <- hclust(d=d, method="ward")   
plot.new()
plot(fit, hang=-1)
groups <- cutree(fit, k=2)   # "k=" defines the number of clusters you are using, k = 5 is original. error says must be between 0and 2   
...
library(fpc)   
library(cluster)  
dtms <- removeSparseTerms(dtm, 0.6) # Prepare the data (max 15% empty space)   
